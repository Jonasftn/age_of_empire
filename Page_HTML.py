...
class Page_HTML:
    ...
    def generate_html(self, persons, ressources, buildings):
        ...
        for joueur in joueurs_tries:
            for joueurs, data in compteurs_joueurs.items():
                if joueur == joueurs:
                    html += f'<div id="{joueur}" class="tab">'
                    html += f"<h2>Unité du joueur {joueur}</h2>"
                    html += "<table><tr><th>Type de Ressources</th><th>Nombre de ressources</th></tr>"
                    print(compteurs_joueurs)

                    for ressource, valeur in data['ressources'].items():
                        ressource_name = {
                            'W': 'Wood', 'f': 'Food', 'G': 'Gold', 'U': 'Unit Cap', 'max_pop': 'Max Population'
                        }.get(ressource, ressource)

                        html += f"<tr><td>{ressource_name}</td><td>{valeur}</td></tr>"

        ...
        while self.terminal_active:
            ...
            for row in range(view_top, min(view_top + max_rows, map_rows)):
                for col in range(view_left, min(view_left + max_cols, map_cols)):
                    ...
                    color = 0  # Pas de couleur

                    # Afficher le joueur
                    if (row, col) == (player_x, player_y):
                        char = "P"
                        color = 12  # Couleur spéciale pour le joueur

                    # Vérifier les bâtiments
                    building = self.buildingsDict.get((col, row))
                    if building:
                        char = building.entityType
                        color = self.get_player_color(building.playerName)
    

                    for person in self.persons:
                        if hasattr(person, "position"):  # Vérifiez si l'objet a bien un attribut 'position'
                            if person.position == (col, row):
                                char = person.entityType
                                color = self.get_player_color(person.playerName)
                                break  # On sort dès qu'on trouve la personne

                    # Vérifier les ressources
                    ressource = self.ressourcesDict.get((col, row))
                    if ressource:
                        if ressource.entityType == "G":
                            char = "G"
                        elif ressource.entityType == "W":
                            char = "W"

                    # Afficher la tuile dans le terminal
                    try:
                        if color != 0:
                            stdscr.addstr(row - view_top, col - view_left, char, curses.color_pair(color))
                        else:
                            stdscr.addstr(row - view_top, col - view_left, char)

                    except curses.error:
                        pass  # Ignorez les erreurs si la position est hors écran
